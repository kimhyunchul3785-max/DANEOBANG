...
export default async function ScanReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <Link href="/app/scans" className="text-xs text-slate-500 hover:underline">
          ← 사진 채점
        </Link>
        <h1 className="h1">판독 검수: {scan.fileName}</h1>
        <p className="muted">
          상태 <b>{scan.status}</b> · {fmtDate(scan.createdAt)}
          {a && (
            <>
              {" "}
              · <b>{a.student.name}</b> · {a.exam.title} · 페이지 {scan.page!.pageNo}/{scan.page!.print.pages.length}
            </>
          )}
        </p>
      </div>
