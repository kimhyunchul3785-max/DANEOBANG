...
export default async function ExamDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ genError?: string; form?: string; published?: string; assigned?: string; step?: string }> }) {
  ...
  // 공통 마감: 안 친 학생 기준, 모두 완료했으면 전체 기준 (완료 뒤 "마감 없음"으로 보이지 않게)
  ...
  // 시작: 전부 이미 열렸으면(시각이 몇 초씩 달라도) "시작됨 · 첫 시작 시각", 예약이 하나면 "예약", 예약이 여럿이면 "학생마다 다름"
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]" data-testid="step-targets">
          ...
          <div>
            {published.length > 0 ? (
              <AssignPanel
                examId={exam.id}
                forms={published.map((f) => ({ id: f.id, version: f.version }))}
                students={students.filter((s) => !exam.assignments.some((a) => a.studentId === s.id)).map((s) => ({ id: s.id, name: s.name, className: s.classRoom?.name ?? null }))}
                /* 반 목록: 아직 대상이 아닌 (내가 배정할 수 있는) 학생이 있는 반만, 인원과 함께 */
                classes={classes
                  .map((c) => ({ id: c.id, name: c.name, count: students.filter((s) => s.classId === c.id && !exam.assignments.some((a) => a.studentId === s.id)).length }))
                  .filter((c) => c.count > 0)}
                defaultDue={commonDue}
              />
            ) : (
        ...
        <div className="space-y-4" data-testid="step-run">
          <div className="bento">
            <section className={`${published.length ? "card-dark" : "card-accent"} span-2 card-body`}>
              ...
              {exam.assignments.length > 0 && (
                <p className="mt-1 text-[12.5px]" style={{ color: "rgba(236,233,227,0.75)" }} data-testid="period-line">
            ...
            <section className="card span-2 card-body" id="due">
              ...
              <p className="muted mt-1">{commonDue ? `현재 마감 ${fmtDate(commonDue)}${openOnes.length === 0 && exam.assignments.length > 0 ? " · 모두 완료" : ""}` : dueList.length > 1 ? "학생마다 마감이 다릅니다" : "마감 없음"}</p>
