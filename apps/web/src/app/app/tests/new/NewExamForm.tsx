 ...
 */
export function NewExamForm({ books, defaultBookId, defaultDays, classes, isOwner = true }: { books: Book[]; defaultBookId?: string; defaultDays?: number[]; classes: Cls[]; isOwner?: boolean }) {
  ...
  const [duePreset, setDuePreset] = useState<"week1" | "today" | "sunday" | "custom" | "none">("week1");
  ...
  const pickDue = (p: typeof duePreset, base = effectiveStart) => {
    ...
    if (p === "sunday") setDue(seoulDate(sundayOffset()));
    if (p === "none") setDue("");
  };
    ...
    <form action={action} className="grid gap-5 lg:grid-cols-5">
      ...
      <div className="space-y-5 lg:col-span-3">
        ...
        <section className="card card-body">
          ...
          <p className="muted mt-2">
            {isOwner ? "고른 반 전원에게 나갑니다." : "고른 반의 내 담당 학생에게 나갑니다 (숫자 = 담당 학생 수)."} 사람 단위 조정은 출제 뒤 <b>시험 상세 → 응시 대상</b>에서.
          </p>
        </section>

        {/* 3. 조건 */}
        <section className="card card-body">
          <StepHead n={3} title="조건" hint="몇 문항 · 몇 점이면 통과 · 단어당 몇 초 · 언제부터 언제까지" />
            ...
            <div className="grid gap-5 sm:col-span-2 sm:grid-cols-2" data-testid="period">
              <div>
                ...
                {startPreset === "custom" ? (
                  ...
                  <p className="muted mt-2">출제하는 순간부터 학생 앱에 나가요.</p>
                )}
              </div>
              <div>
                ...
                {duePreset !== "none" && <input type="datetime-local" value={due} onChange={(e) => { setDue(e.target.value); setDuePreset("custom"); }} className="input mt-2" aria-label="마감 일시" data-testid="due-input" />}
                {dueError ? (
                  <p className="mt-2 text-[12.5px] font-semibold" style={{ color: "var(--accent)" }} role="alert" data-testid="due-error">
                    {dueError}
                  </p>
                ) : (
                  <p className="muted mt-2">{duePreset === "week1" ? "시작부터 7일 · 시각 그대로" : duePreset === "none" ? "기한 없이 열어 둡니다." : "지난 뒤에도 시험 상세에서 늘릴 수 있어요."}</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="space-y-3 lg:col-span-2 lg:sticky lg:top-6 lg:self-start">
        <div className="rounded-[26px] p-3" style={{ background: "var(--charcoal)" }} data-testid="preview-runner">
          ...
          <div className="px-2 pb-1 pt-3 text-[12px]" style={{ color: "rgba(236,233,227,0.8)" }}>
            <b>{range}</b> · {count}문항 · 통과 {pass}점 · 단어당 {perWord}초{timeLimit ? ` · 전체 ${timeLimit}분` : ""} · {startLabel} · {dueLabel} · 대상 {targets}명
      ...
      <div className="lg:col-span-5">
        <ActionBar
          testId="compose-bar"
          note={
            sel.length === 0 ? "1 · 범위에서 DAY를 고르세요" : dueError ? "3 · 마감을 시작 뒤로 잡아 주세요" : `${range} · ${Math.min(count, available || count)}문항 · 통과 ${pass} · ${perWord}초 · ${startLabel} · ${dueLabel} · 대상 ${targets}명`
          }
        >
          <button className="btn-ghost" disabled={pending || sel.length === 0 || !!dueError} name="publishNow" value="off" data-testid="compose-draft">
            초안만 저장
          </button>
          <button className="btn-primary" disabled={pending || sel.length === 0 || !!dueError} name="publishNow" value="on" data-testid="compose-submit">
