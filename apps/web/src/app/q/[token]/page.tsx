...
export default async function QrPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const shell = (children: React.ReactNode) => (
    <main className="mx-auto w-full max-w-[420px] px-4 pb-12 pt-6">
      <div className="mb-4 flex items-center justify-between px-1">
        <Logo height={20} />
        <span className="digital">종이 시험 · QR</span>
      </div>
      {children}
    </main>
  );
      ...
      return shell(
        <>
          ...
          <div className="card card-body anim-fade-up" data-testid="teacher-submit">
            <div className="lbl">대신 제출</div>
            <div className="mt-1 text-[15px] font-semibold">{student.name} 학생 시험지 · 아직 제출 전</div>
            <p className="muted mt-1">보통은 학생이 직접 QR을 찍어 올립니다. 학생이 못 올릴 때만 여기서 대신 찍어 올리면 바로 판독돼요. 결과는 사진 채점(대조) 화면과 성적에 들어갑니다.</p>
            <div className="mt-3">
              <TeacherSubmit />
            </div>
            <Link href={`/app/tests/${exam.id}`} className="lbl-ink mt-3 inline-block">
              시험 상세 →
            </Link>
          </div>
        </>,
  ...
  if (grade) {
    ...
    const showScore = exam.scoreVisibility === "immediate" || exam.answersReleased;
    ...
    return shell(
      <>
        {showScore ? (
          <div className={`${grade.passed ? "card-dark" : "card-accent"} card-body anim-fade-up`}>
            <div className="flex items-start justify-between">
              <div className="lbl" style={{ color: grade.passed ? "rgba(236,233,227,0.55)" : "rgba(255,244,240,0.8)" }}>
                {grade.passed ? "통과" : "미달"} · {student.name}
              </div>
          ...
          <div className="card-dark card-body anim-fade-up">
            <div className="lbl" style={{ color: "rgba(236,233,227,0.55)" }}>채점 완료 · {student.name}</div>
            <div className="mt-2 text-[18px] font-semibold">점수는 선생님이 공개한 뒤 보여요</div>
            <div className="mt-1 text-[13px]" style={{ color: "rgba(236,233,227,0.8)" }}>{exam.title}</div>
          </div>
        )}
        <section className="card card-body mt-3">
          <div className="flex items-center justify-between">
            <div className="lbl">틀린 문항</div>
