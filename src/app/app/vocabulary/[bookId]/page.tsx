  ...
  const examRow = (e: (typeof exams)[number]) => {
    ...
    return (
      <li key={e.id} className="row">
        <Link href={`/app/tests/${e.id}`} className="min-w-0 flex-1 hover:underline">
      ...
      <section className="card mt-4" data-testid="book-exams">
        <div className="card-body">
          <div className="mb-2 flex items-center justify-between">
            <div className="lbl">Tests · 이 단어장으로 만든 시험</div>
            <span className="digital">{exams.length}</span>
          </div>
          {exams.length === 0 ? (
            <p className="muted">아직 없습니다. 위의 "시험 만들기"로 이 단어장에서 출제하세요.</p>
          ) : (
            <>
