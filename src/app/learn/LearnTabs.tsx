 ...
 * 응시 중(/learn/attempts)에는 둘 다 숨긴다. 탭 변형은 결과·연습 화면에서도 숨긴다(뒤로 가기 링크가 있음).
 */
export function LearnTabs({ variant = "tabs" }: { variant?: "tabs" | "side" }) {
  ...
  if (variant === "tabs" && (p.startsWith("/learn/results") || p.startsWith("/learn/practice"))) return null;
  const isOn = (href: string) => (href === "/learn" ? p === "/learn" || p.startsWith("/learn/results") : p.startsWith(href));
  if (variant === "side") {
    return (
