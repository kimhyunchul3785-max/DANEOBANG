 ...
 */
export type SeatUsage = { quantity: number; used: number; pending: number; available: number; unitPrice: number; monthly: number; status: string | null };
...
export async function seatUsage(academyId: string): Promise<SeatUsage> {
  ...
  const quantity = sub?.seatQuantity ?? 0;
  const unitPrice = sub?.unitPrice ?? 9900;
  return { quantity, used, pending, available: Math.max(0, quantity - used - pending), unitPrice, monthly: quantity * unitPrice, status: sub?.status ?? null };
}
