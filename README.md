...
## v5.8 — 차트 시스템 · 덜 그리기 (1차 Visual Pass)

> v5.6 기준으로 만들었다. 같은 v5.6 에서 나온 v5.7 패치(메뉴 6개 · 모바일 4탭)와는 `app/page.tsx` · 시험 상세 · `globals.css` 가 겹친다.

- **차트 컴포넌트 재정의** (`components/Viz.tsx`): `TrendChart`(상세 추이) · `MicroTrend`(카드·표 안 보조 추이) · `RankBars`(비교) · `Distribution`(5구간 분포) · `CompositionBar`(구성비, 도넛 대체) · `Heatmap`(4단계 구간 색) · `Meter` · `Ring`(QR 결과만). `Sparkline` · `HBars` · `Columns` · `Donut` · `PulseBars` 제거.
- **선 차트**: 선만 SVG 로 늘리고 점·라벨·툴팁은 HTML(% 위치) → 폭이 바뀌어도 점이 찌그러지지 않음. 단조 곡선, 점은 마지막 값 + (드물 때만) 미달 점, 끝 값 직접 표기, hover/포커스 툴팁. 점수 축은 최소 40점 폭.
- **성적 요약**: 숫자 4개는 한 표면의 띠, 카드 높이는 내용에 맞춤(같은 줄끼리 정렬). 기본 배치 = 추이 2/3 + 챙겨야 할 학생 1/3 → 반별 평균 · 점수 분포 · 최근 시험. 히트맵은 기본에서 빠짐(편집에서 넣을 수 있음). **레이아웃 편집은 머리의 ⋯ 메뉴 안**으로. 저장된 배치는 그대로.
- **원장 현황**: 도넛 → 구성 막대, 오른쪽은 '이번 주 결과 + 8주 평균(숫자가 주인공)' 한 표면. 반별 표의 8주 미니 추이는 같은 세로 범위.
- **학생·반 상세**: 학생별 → 학생 선택 화면에 점수 추이 추가, 평균 아래 '최근 k주 ±n'. 학생 상세는 추이를 기록 맨 위로.
- **배지 줄이기**: 날짜 · D-day · 분류(재시험) · 개수 · 공개 정책은 글자로, 상태만 배지. **휴대폰 시험 상세**는 큰 숫자 하나(완료) + 한 줄 요약.
- 테스트: `e2e-retake` 는 ⋯ 메뉴로 편집에 들어가고, `e2e-compose` 는 공개 정책을 글자로 확인.

## v5.2 — UX QA 반영: 로그인 500 · 러너 준비 화면 · 휴대폰 하단 탭 · 폴더 · 예약 알람

- **P1**: 로그인 상태에서 `/login` 500 (쿠키 굽기를 Route Handler 로), 학생 탭 반 칩 = 담당 학생 수, 러너 준비 화면 → [시작] → 3·2·1 · 0.4초 안 되돌리기 · 시간 초과 플래시.
- **단어장**: 보관·열기 제거, 카드 전체 클릭, ⋯ 메뉴(폴더로 이동 · 삭제), **폴더**(1단계), DAY 나누기는 일수·단어 수만.
- **출제**: 마감 1주 뒤·직접·없음, 대상 반을 골라야 출제(초안은 예외), 시간 제한은 조건으로, 스텝퍼, 같은 이름 시험은 `(9/24 · 2차)`. **예약 출제는 시작 시각에 학생 알림**(종 + 앱 푸시 + 브라우저 알림).
- **공개 4케이스**: 학생 결과 카드에 점수/정답 정책 칩, 시험 상세에 현재 상태 + 공개 버튼, 공개 시 학생 알림. 점수 정책이 새던 곳(이번 주·종이·재시험·QR·알림) 정리.
- **사진 채점** = 학생 제출 대조 화면(업로드 없음). **성적** 요약/학생별/시험별 3탭. **재시험** 학생별 묶음 · 한 줄 요약 · "재시험 없이 종료" · 오래된 항목 정리. **오늘** 숫자·라벨 정리.
- **휴대폰**: 하단 탭 바 + ⋯ 메뉴, 표 9곳 카드형(`tbl-cards`). 학생 앱 값·상태·버튼 한국어. 랜딩 "무료 체험 중" 제거.
- DB: `BookFolder` · `VocabBook.folderId` (`start-server.bat` / `pnpm run db:push` 가 자동 추가). 결과: `docs/QA_REPORT_v5.2.md`.

## v5.1 — IA 정리: 부모/자식 위계 · 출제 두 경로 · 단어장 병합

- **단어장 상세**: 우측 상단 검정 [시험 만들기]·[이 DAY로 출제] 없음. 하단 바 하나 **이 단어장으로 시험 만들기 →** → `/app/vocabulary/{id}/new-test` (단어장 고정, 지금 보는 DAY 미리 선택). 이 단어장의 시험은 아래 요약 + "→ 시험 탭".
- **시험 탭 → [시험 만들기]** 는 위저드: 단어장 → 범위 → 대상 → 조건 → 이름·공개 순서로 한 단계씩(이전/다음). 단어장에서 들어온 출제 화면과 다르다.
- **시험 상세** 는 진행 화면 하나: 진행 요약 · 성적 요약(평균·통과율·미달) 인라인 · 마감기한 변경 · 종이 시험지 · 정답지. 문항·정답과 응시 대상은 자식 페이지(`/items` · `/targets`). 초안은 "검토 후 발행" 카드.
- **단어장 병합**: 목록 카드에서 [병합] 체크 → 아래 바 → 순서·겹치는 단어(표제어 같으면 뒤 단어장 것 제거) 미리보기 → 제목·DAY(이어 붙이기/다시 나누기)·원본 보관 → 새 단어장.
- **성적 탭** 상세 필터는 고르면 바로 렌더([조회] 없음). 오늘·성적·재시험·학생 상세의 깊은 링크는 "→ 탭 + 필터"로 (A→B, C→B 건너뛰기 없음). `/app/scans` → `/app/tests/scans`, `/app/imports/[id]` → `/app/vocabulary/imports/[id]` (옛 주소 redirect).
- 시험 목록 필터(진행 중·마감 지남·초안·보관) + 검색 + `?bookId=`. 문서: `docs/IA_REVIEW_v5.md` (현재 IA → 문제점 → 권장 위계 → 버튼별 이동 경로 → 사이드바/탭/버튼 구분 → 구현 결과).
- 스크립트: `pnpm run e2e:compose`(72 체크) · `pnpm run e2e:merge`(21 체크) · `pnpm run qa:visual`(72장) — `apps/web` 에서.
...
pnpm run e2e:compose    # v5.2 출제 흐름 95 체크: 업로드 → 단어장 고정 출제 → 시험 상세(진행)·items·targets → 학생 응시 → 시험 탭 위저드 → 예약 → 초안 발행 → 목록 필터 → 성적 즉시 렌더 (log/e2e-compose)
pnpm run e2e:merge      # v5.2 단어장 병합·폴더·삭제 27 체크: 카드 체크 → 병합 바 → 겹침 미리보기·순서 → 이어 붙이기/다시 나누기 → 폴더 만들기·이동·이름·지우기 → 삭제 (log/e2e-merge)
pnpm run qa:visual      # v5.2 페이지별 시각 QA: 역할별 43화면 × 데스크톱·휴대폰 촬영 + 넘침·잘림·오류 자동 검사 (log/qa-visual)
...
- `docs/QA_REPORT_v5.2.md` — v5.2 UX QA 리포트(P1·P2 + 메모 9건) 반영 결과와 검증
- `docs/IA_REVIEW_v5.md` — v5.1 IA 정리: 현재 IA·문제점·권장 부모/자식 위계·버튼별 이동 경로·사이드바/탭/버튼 구분·구현 결과와 회귀
